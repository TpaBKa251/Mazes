...
@DisplayName("Тесты для класса RendererMazeImplTest")
public class RendererMazeImplTest {

    RendererMaze rendererMaze = new RendererMazeImpl();
    Maze maze = createMaze();

    @DisplayName("Тест преобразования лабиринта в строку")
    @Test
    void testRenderMaze() {
        String expectedMazeString = """
            ██████████
            █ @█ ~/█ █
            ██░ #█ █═█
            █ @@ /█ ██
        ...
        String actualMazeString = rendererMaze.renderMaze(maze);

        assertThat(actualMazeString).isEqualTo(expectedMazeString);
    }

    @DisplayName("Тест преобразования лабиринта в строку с добавлением пути")
    @Test
    void testRenderMazeWithPath() {
        String expectedMazeWithPath = """
            ██████████
            █Ax█ ~/█ █
            ██x #█ █═█
            █ x@ /█ ██
        ...
        String actualMazeWithPath = rendererMaze.renderMazeWithPath(maze, getPath())
            .replaceAll("\u001B\\[[;\\d]*m", "");

        assertThat(actualMazeWithPath).isEqualTo(expectedMazeWithPath);
    }
